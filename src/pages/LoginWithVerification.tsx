...
function LoginWithVerification() {
        ...
        try {
            ...
            if (userProfile) {
                ...
                if (userProfile.password === hashedInputPassword) {
                    console.log('Password match! Sending verification code...');
    ...
    const handleVerificationSuccess = () => {
        if (pendingUserData) {
            // Finaliser la connexion après vérification réussie
            localStorage.setItem('currentUser', JSON.stringify(pendingUserData));
            localStorage.setItem('isLoggedIn', 'true');
            localStorage.setItem('currentUserRole', pendingUserData.role);
            localStorage.setItem('loginTime', Date.now().toString());
            
            loadThemeForUser(pendingUserData.email);
            navigate('/home');
        }
    };
    ...
    // Si on est en mode vérification, afficher le composant de vérification
    if (showVerification) {
        return (
            <div style={{ 
                minHeight: '100vh', 
            ...
            <div style={{ flex: 1, display: 'flex', flexDirection: 'column', justifyContent: 'center', padding: '3rem 2rem' }}>
                <div style={{ width: '100%', maxWidth: '450px', margin: '0 auto' }}>
                    ...
                    <p style={{ color: 'var(--text-secondary)', marginBottom: '2rem', fontSize: '0.85rem' }}>
                        🔒 Chiffrement AES-256 + Vérification par email
                    </p>
                    <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                        <div>
                            ...
                            <input 
                                ...
                                placeholder="votre.email@ols.com" 
                                required 
                                style={{ width: '100%', padding: '1rem', border: '2px solid var(--border-color)', borderRadius: '0.75rem', fontSize: '1rem', background: 'var(--card-bg)', color: 'var(--text-primary)' }} 
                            />
                        </div>
                        <div>
                            ...
                            <div style={{ position: 'relative' }}>
                                <input 
                                    ...
                                    placeholder="••••••••" 
                                    required 
                                    style={{ width: '100%', padding: '1rem', paddingRight: '3rem', border: '2px solid var(--border-color)', borderRadius: '0.75rem', fontSize: '1rem', background: 'var(--card-bg)', color: 'var(--text-primary)' }} 
                                />
                                <button 
                                    type="button" 
                        ...
                        {error && (
                            <div style={{ color: '#ef4444', fontSize: '0.9rem', padding: '0.75rem', background: 'rgba(239, 68, 68, 0.1)', borderRadius: '0.5rem', border: '1px solid rgba(239, 68, 68, 0.2)' }}>
                                {error}
                            </div>
                        )}
                        <button 
                            type="submit" 
                            disabled={loading} 
                            style={{ width: '100%', padding: '1rem', background: loading ? '#999' : 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', color: 'white', border: 'none', borderRadius: '0.75rem', fontSize: '1.1rem', fontWeight: 600, cursor: loading ? 'not-allowed' : 'pointer' }}
                        >
